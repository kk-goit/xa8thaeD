...
<li class="exercise-card" data-id=${s._id}>
  ...
  `,Q=t=>t.map(K).join(""),V=t=>{const e=document.querySelector(".group-list");if(!e)return;const s=Q(t);e.innerHTML=s},W=async t=>await y.getExercisesByFilter(t),b=async({filter:t="Muscles",page:e=1,limit:s=12}={})=>{const r=await W({filter:t,page:e,limit:s});e=E(),L=t.toLowerCase(),V(r.results),p(r.totalPages,b,L)},H=Array.from(document.querySelectorAll(".exercises-menu-button")),Y=t=>{const e=document.querySelector(".section-title");e.innerHTML="Exercises",H.forEach(s=>{s.classList.remove("active")}),t.classList.add("active"),b({filter:t.textContent})};H.forEach(t=>t.addEventListener("click",()=>Y(t)));export{y};
//# sourceMappingURL=main-CJs1ix4r.js.map
