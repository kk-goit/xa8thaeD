...
<li class="exercise-card" data-id=${n._id}>
  ...
  `,Ie=t=>t.map(He).join(""),Fe=t=>{const e=document.querySelector(".group-list");if(!e)return;const s=Ie(t);e.innerHTML=s},Ae=async t=>await b.getExercisesByFilter(t),R=async(t="Muscles",e=1,s=screen.width>767?12:9)=>{t=t.trim(),e=J(),_=t.toLowerCase(),_==="body parts"&&(_="bodypart"),A(document.querySelector(".group-list"));const r=await Ae({filter:t,page:e,limit:s});Fe(r.results),H("category"),Le(r.totalPages,R,t)},re=Array.from(document.querySelectorAll(".exercises-menu-button")),De=t=>{const e=document.querySelector(".section-title"),s=document.querySelector(".exercises-form");e.innerHTML="Exercises",re.forEach(r=>{r.classList.remove("active")}),t.classList.add("active"),s.classList.add("visually-hidden"),fe(),R(t.textContent)};re.forEach(t=>t.addEventListener("click",()=>De(t)));const P=document.getElementById("scrollToTop");window.addEventListener("scroll",()=>{window.scrollY>300?P.style.display="flex":P.style.display="none"});P.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})});export{se as a,X as b,A as r,h as t,b as y};
//# sourceMappingURL=main-BANR_apO.js.map
